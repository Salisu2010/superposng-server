...
// Create/Update shop alias (old -> canonical)
r.post("/shops/alias", requireDevKey, (req, res) => {
  const { fromShopId, toShopId } = req.body || {};
  const from = (fromShopId || "").toString().trim();
  const to = (toShopId || "").toString().trim();
  if (!from || !to) return res.status(400).json({ ok:false, error:"fromShopId and toShopId are required" });
  const db = readDB();
  if (!Array.isArray(db.shopAliases)) db.shopAliases = [];
  const now = Date.now();
  const ex = db.shopAliases.find(a => a.from === from);
  if (ex) { ex.to = to; ex.updatedAt = now; }
  else db.shopAliases.push({ from, to, createdAt: now, updatedAt: now });
  writeDB(db);
  return res.json({ ok:true, from, to });
});

function _mergeArrayByKey(into, fromArr, keyFn) {
  const seen = new Set(into.map(x => keyFn(x)));
  for (const it of fromArr) {
    const k = keyFn(it);
    if (!seen.has(k)) { into.push(it); seen.add(k); }
  }
}

function mergeShopData(db, fromId, toId) {
  const now = Date.now();
  if (!Array.isArray(db.shopAliases)) db.shopAliases = [];
  const ex = db.shopAliases.find(a => a.from === fromId);
  if (ex) { ex.to = toId; ex.updatedAt = now; }
  else db.shopAliases.push({ from: fromId, to: toId, createdAt: now, updatedAt: now });

  const tables = ["products","staffs","sales","debtors","devices","pairCodes","licenses","pendingActivations"];
  for (const t of tables) if (!Array.isArray(db[t])) db[t] = [];

  // Move records with shopId=fromId to toId, dedupe safely.
  const move = (t, keyFn) => {
    const src = db[t].filter(x => (x.shopId||"") === fromId);
    const dst = db[t].filter(x => (x.shopId||"") === toId);
    const others = db[t].filter(x => (x.shopId||"") !== fromId && (x.shopId||"") !== toId);
    for (const it of src) it.shopId = toId;
    _mergeArrayByKey(dst, src, keyFn);
    db[t] = [...others, ...dst];
  };

  move("products", p => (p.productId||"") || ((p.barcode||"")+"|"+(p.sku||"")+"|"+(p.name||"")));
  move("staffs", s => (s.staffId||"") || ((s.phone||"")+"|"+(s.name||"")));
  move("sales", s => (s.saleId||"") || ((s.receiptNo||"")+"|"+(s.createdAt||s.ts||"")+"|"+(s.total||"")));
  move("debtors", d => (d.debtorId||"") || ((d.phone||"")+"|"+(d.name||"")));

  // devices: keep unique by deviceId
  const srcDev = db.devices.filter(d => (d.shopId||"")===fromId);
  const dstDev = db.devices.filter(d => (d.shopId||"")===toId);
  const otherDev = db.devices.filter(d => (d.shopId||"")!==fromId && (d.shopId||"")!==toId);
  for (const d of srcDev) d.shopId = toId;
  _mergeArrayByKey(dstDev, srcDev, d => (d.deviceId||""));
  db.devices = [...otherDev, ...dstDev];

  // Mark source shop as aliasOf toId (do not delete to avoid breaking references)
  const sFrom = (db.shops||[]).find(s => s.shopId === fromId);
  if (sFrom) { sFrom.aliasOf = toId; sFrom.isDisabled = true; sFrom.updatedAt = now; }

  // Update lastSyncedAt if present
  const sTo = (db.shops||[]).find(s => s.shopId === toId);
  if (sTo && sFrom && sFrom.lastSyncedAt && !sTo.lastSyncedAt) sTo.lastSyncedAt = sFrom.lastSyncedAt;

  return true;
}

// Merge one shop into another (moves data + creates alias)
r.post("/shops/merge", requireDevKey, (req, res) => {
  const { fromShopId, toShopId } = req.body || {};
  const from = (fromShopId || "").toString().trim();
  const to = (toShopId || "").toString().trim();
  if (!from || !to || from === to) return res.status(400).json({ ok:false, error:"fromShopId and toShopId are required and must differ" });
  const db = readDB();
  const hasFrom = (db.shops||[]).some(s => s.shopId === from);
  const hasTo = (db.shops||[]).some(s => s.shopId === to);
  if (!hasFrom || !hasTo) return res.status(404).json({ ok:false, error:"Shop not found (from or to)" });
  mergeShopData(db, from, to);
  writeDB(db);
  return res.json({ ok:true, from, to });
});
