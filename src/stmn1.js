...
// ------------------------------------------------------------
// StayMasterNG STMN1 Offline Token utilities
// Token format:
//   STMN1|PLAN|YYYYMMDD|DEVHASH16|SIG12
// DEVHASH16 = sha256(android_id.lower().trim())[:16].upper()
// SIG12     = hmac_sha256(SECRET, payload)[:12].upper()
// payload   = STMN1|PLAN|YYYYMMDD|DEVHASH16
// ------------------------------------------------------------

const PREFIX = "STMN1";

function s(v) { return (v === null || v === undefined) ? "" : String(v); }
export function trim(v) { return s(v).trim(); }

export function stmnSecret() {
  return trim(process.env.STMN_SECRET) || "DAURAWA_STAYMASTERNG_PRIVATE_2026";
}

export function sha256HexUpper(data) {
  return crypto.createHash("sha256").update(trim(data).toLowerCase(), "utf8").digest("hex").toUpperCase();
}
...
// Nigeria is UTC+1, no DST. Use Africa/Lagos to compute dates.
...
export function genStmn1Token(plan, androidId, ymdOverride = "") {
  ...
  const sig12 = hmacSha256HexUpper(stmnSecret(), payload).slice(0, 12);
...
export function parseAndVerifyStmn1(tokenRaw) {
  ...
  const expected = hmacSha256HexUpper(stmnSecret(), payload).slice(0, 12);
