...
function currentServer() {
  const srv =
    normBase(state.serverUrl) ||
    normBase(elv("serverUrl")) ||
    normBase(localStorage.getItem("tg_server") || "");
  state.serverUrl = srv;
  return srv;
}
...
function currentKey() {
  const key =
    String(state.adminKey || "").trim() ||
    String(elv("adminKey") || "").trim() ||
    String(localStorage.getItem("tg_admin") || "").trim();
  state.adminKey = key;
  return key;
}
